...
<Tldr>vLLM gets its throughput from a tight engine loop that batches prefill and decode separately, plus a paged KV cache that behaves like OS memory. The result is higher utilization, less fragmentation, and better reuse of prefixes.</Tldr>
