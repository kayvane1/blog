...
<Tldr>
  A document ingestion service has to resolve "Apple Inc." to a stable internal ID — exactly one ID,
  forever, regardless of how many containers, threads, or weeks have passed. Three cache layers
  shrink the latency cost from "every lookup is a database round-trip" down to "the popular ones
  never leave the Python process." L1 (in-process LRU) collapses the 16 threads on one container
  into one shared answer. L2 (Modal Dict + Queue) collapses the 50 containers into one database
  call. L3 is the database itself — the source of truth, but hundreds of milliseconds away over
  the network. The pattern has a name: _singleflight with a distributed lock and a notify channel_.
</Tldr>
...
| L3    | 100 – 300 ms    | Database (network hop outside Modal) |

Each step you avoid is worth an order of magnitude (or two) less time. The lock matters because under a stampede, you're not just saving compute — you're collapsing N expensive L3 round-trips into one.
...
- **The hop itself.** L3 sits across whatever network boundary separates Modal from your DB (peering, VPC, public internet). Hundreds of milliseconds is normal; more than that is a sign the DB is in the wrong region. Pin both sides to the same region if you can.
